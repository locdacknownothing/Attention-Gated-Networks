...
class STAREDataset(data.Dataset):
    def __init__(self, root_dir, split, transform=None, preload_data=False, seed=42, target_size=(704, 704)):
        ...
        all_image_files = sorted([f for f in listdir(image_dir) if f.endswith('.png')])

        if split in ['train', 'validation']:
            # Group files by base image ID (e.g. im0001, im0002, etc.) to prevent data leakage
            base_groups = {}
            for img_file in all_image_files:
                match = re.search(r'im\d+', img_file)
