...
class CHASEDataset(data.Dataset):
    def __init__(self, root_dir, split, transform=None, preload_data=False, seed=42, target_size=(1008, 1008)):
        ...
        # Collect all image files (.jpg)
        all_image_files = sorted([f for f in listdir(image_dir) if f.endswith('.jpg')])

        if split in ['train', 'validation']:
            # Group files by base image ID (e.g. Image_01L, Image_01R, etc.) to prevent data leakage
            base_groups = {}
            for img_file in all_image_files:
                match = re.search(r'Image_\d+[LR]', img_file)
