...
def get_network(name, n_classes, in_channels=3, feature_scale=4, tensor_dim='2D',
    ...
    if tensor_dim == '2D' and isinstance(attention_dsample, (tuple, list)) and len(attention_dsample) > 2:
        attention_dsample = tuple(attention_dsample[:2])
    ...
    if name in ['unet', 'unet_ct_dsv']:
        ...
        if nonlocal_mode == 'embedded_gaussian':
